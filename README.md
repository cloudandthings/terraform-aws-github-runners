...
| <a name="input_environment_variables"></a> [environment\_variables](#input\_environment\_variables) | List of environment variables to set for the CodeBuild environment. Valid types are `PLAINTEXT`, `PARAMETER_STORE`, and `SECRETS_MANAGER`. | <pre>list(object({<br/>    name  = string<br/>    value = string<br/>    type  = string<br/>  }))</pre> | `[]` | no |
...
| Name | Version |
| ---- | ------- |
| <a name="provider_aws"></a> [aws](#provider\_aws) | >= 6, < 7 |
